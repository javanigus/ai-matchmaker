...
```
A pending draft is just the optional second half of a category's existing row (nullable `pending_*` columns) rather than a separate table — there's only ever one draft per category, and "does this category have a pending update" is a single-row check, not a join. `full_summary` sits outside the pending/approved split entirely — it's written directly on every extraction, since it's never rendered on the public profile and never read by matching, only by the user themselves via the "See everything I've picked up" link.

Required fields (`users` columns above, plus `user_ethnicities`) aren't part of this pending/approved machinery at all — no AI call ever proposes a value for them, so there's nothing to draft or approve. They're written directly by a plain form, validated only on Publish (see UX walkthrough below).
