...
**The AI Memory narrative paragraph is the one part that's deferred until baseline, though — not fired per session-close like the rest.** Session-close extraction normally produces two things at once (see "Two different LLM calls" above): the category updates and one narrative paragraph for AI Memory. During onboarding, only the category-update half runs per-turn; the paragraph half is held back until baseline is reached, then generated once over the onboarding conversation so far. Firing it at every session-close during an in-progress, possibly multi-session onboarding would scatter several fragmentary "partial onboarding" entries across AI Memory before the interview is even done — one consolidated entry once there's an actual "how we got started" story to tell reads better and is simpler to reason about. If a user abandons onboarding permanently, no paragraph is ever generated for that attempt, which is correct — there's nothing to summarize yet.
...
**Routing before baseline is reached.** Every page whose content depends on a profile existing — My Profile, AI Memory, AI Recommendations, Matches, Messages, Compatibility Reports — checks `baseline_reached_at` on load and redirects to the onboarding conversation if it's still null. Search is the one exception and stays reachable regardless (see `prd.md` → AI Interview), matching the home page's "browse profiles first" entry point and the Free tier's unlimited manual Search. The persistent AI Matchmaker panel used everywhere else in the product (see UX walkthrough step 9 below) also only appears once baseline is reached — before that, Search shows a lightweight banner/CTA into the onboarding conversation instead, rather than a second, competing "talk to the AI" surface alongside the dedicated onboarding screen.

Redirecting into onboarding resumes the existing conversation rather than restarting it, and this doesn't require replaying old messages: the same mechanism that lets ordinary post-onboarding conversations avoid re-asking what's already known — the system prompt includes the user's current profile snapshot (see "Live turn-by-turn" above) — is what makes resuming work too. Category-level extraction already writes continuously throughout onboarding, so an abandoned session leaves a real, useful, structured snapshot behind even after the raw messages themselves are eventually deleted (see Data retention below).
...
Raw rows in `messages` are deleted 30 days after `created_at`, via a scheduled job — one simple, uniform rule rather than special-casing by conversation type or user. This is safe because messages are working material, not the durable record: everything meant to persist is already extracted into `profile_categories` and `ai_memory_events` well before the 30-day window closes — session-close extraction happens immediately, and during onboarding specifically, category extraction runs even sooner, live and turn-by-turn (see above). The one deterministic check that reads raw messages, the fabricated-evidence check (see "When extraction fails" above), also runs immediately at extraction time, not 30 days later, so retention doesn't weaken it.

**One accepted tradeoff:** the single AI Memory narrative paragraph for onboarding (see above) is deferred until baseline is reached, generated from whatever onboarding messages are still within the 30-day window at that point. If onboarding is unusually slow — spanning more than 30 days across several sessions — the earliest messages may already be gone, and that final paragraph could miss some early color. The structured category data itself is unaffected, since it was already extracted and stored continuously throughout. Rather than special-case retention for slow onboarding, this is accepted as a minor, rare cost of keeping the retention rule uniform.
