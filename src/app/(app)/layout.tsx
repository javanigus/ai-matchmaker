...
// Shared shell for the real, navigable post-auth pages (Search, AI
// Recommendations, Saved Profiles, Matches, Messages, My Profile, AI
// Memory, Compatibility) — a Next.js route group, so it doesn't add a
// URL segment (this file governs /profile, /search, etc. directly, not
// /app/profile). See components/app-nav.tsx for why this doesn't also
// take over the prototype's full 3-column shell.
//
// The redirect below duplicates each page's own auth check (and most
// of these are further gated on baseline_reached_at in proxy.ts) —
// same defense-in-depth pattern already used elsewhere in this app.
// Needed here regardless since the layout itself queries `users.name`
// for the sidebar footer, which requires a real session to mean
// anything.
//
...
//
// Search is the only one of these pages reachable pre-baseline, so
