...
export type OnboardingInitialState = {
  ...
  progress: { percent: number; categories: ProgressCategory[] } | null;
...
// Per founder request: refreshing the page, or coming back after a
// computer restart, used to lose everything — conversationId/messages/
// progress only ever lived in React state, so a reload silently started
// a brand-new conversation row server-side with an empty transcript,
// even though everything actually learned was already safe in
// profile_categories. The server component (page.tsx) now loads the
// user's real most-recent conversation, its messages, and current
// progress before this component ever mounts, and hands it in here —
// this component just resumes from wherever that turns out to be,
// rather than always starting fresh.
export default function OnboardingChat({ initial }: { initial: OnboardingInitialState }) {
  const [messages, setMessages] = useState<ChatMessage[]>(
    initial.messages.length > 0 ? initial.messages : [{ role: "assistant", content: OPENING_MESSAGE }]
  );
  const [input, setInput] = useState("");
  const [conversationId, setConversationId] = useState<string | undefined>(initial.conversationId);
  const [progress, setProgress] = useState(initial.progress);
  const [sending, setSending] = useState(false);
  const [baselineReached, setBaselineReached] = useState(initial.baselineReached);
  ...
  // Seeded from the resumed progress on load, not just turns taken in
  // this browser tab.
  const [everMetCategories, setEverMetCategories] = useState<Set<string>>(
    new Set((initial.progress?.categories ?? []).filter((c) => c.met).map((c) => c.category))
  );
