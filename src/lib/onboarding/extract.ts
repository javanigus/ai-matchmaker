...
- A message that's entirely about the kind of partner they want (age, appearance, personality traits they're seeking, whether the partner has kids) and says nothing about the user's own life is not signal for ANY category, including relationship_goals — don't guess or default to a plausible-sounding value (e.g. "Marriage") just because the conversation feels like it's building toward a profile. If you can't point to a specific phrase where the user described their own stance, don't propose an update, full stop — an empty updates array is the correct output far more often than an invented one.
...
- Don't invent elaboration, reasoning, or specifics the user didn't actually give — this includes reusing wording from the question you asked them as if they'd said it themselves. If their answer was a bare pick from a list of options with no elaboration ("marriage," in response to being asked to choose one), the summary should say exactly that plainly ("Wants marriage — no further detail given yet.") rather than a fuller-sounding paragraph padded out from the question's own wording. A short, honest summary is correct here; an invented-sounding explanation is fabrication even if it's phrased as "not interested in casual dating" — say only what they actually said.
- A category already at Medium or High confidence should almost never drop back down — but this is a floor on confidence, not a reason to skip updating the text. If it already shows [current confidence: High] or [current confidence: Medium] above, only propose an update if this turn adds genuinely new detail worth folding in (keep confidence the same or raise it) or the user contradicts/changes what's already known (reflect the real change, confidence included). "Already High confidence" is never itself a reason to stay silent on real new detail — a category can be certain about a narrow fact (e.g. a quick_fact pick) while still being thin on actual narrative content, and a genuine elaboration turn should always be folded into full_summary/short_summary even when confidence itself doesn't need to move. Only skip when the turn truly adds nothing beyond what's already captured — a light rephrasing of the same fact, not real new detail like reasons, specifics, or context they hadn't given before.
